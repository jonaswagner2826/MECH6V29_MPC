...
saveas(gcf,strcat('figs',filesep,'results_','open_',roadProfile,'.png'))

%% Discrete-time LQR Control (Optional)
% LQR cost function matrices 
Q = eye(nx); % DESIGN THIS
R = eye(nu); % DESIGN THIS
...
sgtitle(strcat('Simulation Reusults:',...
    'LQR', 'profile = ',roadProfile))
saveas(gcf,strcat('figs',filesep,'results_','LQR_',roadProfile,'.png'))
% Animate simulation data (can comment out if you dont want to animate)
